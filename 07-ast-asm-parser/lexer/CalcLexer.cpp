...
const std::string CalcLexer::m_printExpressionName("print");
...
Token CalcLexer::ReadWord(char head)
{
    /*
     * Reads the tail of number token and returns this token.
     * PRECONDITION: first character already read.
    ...
    if (value == m_printExpressionName)
    {
        return Token{TT_PRINT};
    }
    return Token{TT_ID, std::move(value)};
}
